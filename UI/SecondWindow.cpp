...
void SecondWindow::refresh_lionel_stuff() {
    if (musicON_) {
        ...
        ui->verticalSlider_1->setValue(rand() % 100);
        ui->verticalSlider_2->setValue(rand() % 100);
        ui->verticalSlider_3->setValue(rand() % 100);
        ui->verticalSlider_4->setValue(rand() % 100);
        ui->verticalSlider_5->setValue(rand() % 100);
        ui->verticalSlider_11->setValue(rand() % 100);
        ui->verticalSlider_12->setValue(rand() % 100);
        ui->verticalSlider_13->setValue(rand() % 100);
        ui->verticalSlider_14->setValue(rand() % 100);
        ui->verticalSlider_15->setValue(rand() % 100);
    }
    //// Lionel test
    static bool a(false);
    ui->ignition_status_label->setStyleSheet((a) ? "QLabel {color: rgb(255, 255, 255);}"
                                             : "QLabel {color: rgb(255, 0, 0);}");
    ui->antenna_img->setStyleSheet((xbee_acvite_ && a) ?
...
{
    if (!xbee_acvite_) {
        ui->xbee_button->setStyleSheet("QPushButton{\n"
        ...
        uint64_t index(ui->serialport_selector->currentIndex());
...
void SecondWindow::ignite_clicked()
{
    ...
    if (ready_ignition_) {
        ui->ready_ignit_button->setStyleSheet(
            "QPushButton { qproperty-icon: url(:/assets/readiness.png);}");
        ui->red_button->setStyleSheet(
...
void SecondWindow::refresh_ignition_code()
{
    ...
    ui->code_0->setText(QString::number(get_bit(tmp, 0)));
    ui->code_1->setText(QString::number(get_bit(tmp, 1)));
    ui->code_2->setText(QString::number(get_bit(tmp, 2)));
    ui->code_3->setText(QString::number(get_bit(tmp, 3)));
}

void SecondWindow::refresh_av_state()
{
    std::string str(avionic::getAVStateName(connector->getData<uint8_t>(STATUS_AV_STATE)));
    ui->avionics_state_panel->setText(QString::fromStdString(str));
}
...
void SecondWindow::refresh_ignition_frame()
{
    ...
    // Keys
    ui->ignition_key1->setStyleSheet((key1) ? "QLabel {image: url(:/assets/keyON.png);}"
                                        : "QLabel {image: url(:/assets/keyOFF.png);}");
    ui->ignition_key2->setStyleSheet((key2) ? "QLabel {image: url(:/assets/keyON.png);}"
                                        : "QLabel {image: url(:/assets/keyOFF.png);}");

    // Ignition status
    auto ignitState = connector->getData<ignit::IgnitionState>(IGNITION_STATUS);
    ui->ignition_status_label->setText(QString::fromStdString(ignit::getIgnitionState(ignitState)));
    switch (ignitState) {
    case ignit::SLEEP:
        ui->ignition_state_icon->setStyleSheet("QLabel {image: url(:/assets/safe.png);}");
        break;
    case ignit::WRONG_CODE_RECEIVED:
        ui->ignition_state_icon->setStyleSheet("QLabel {image: url(:/assets/password.png);}");
            ui->info_ignition->setText("The GSE informs you that you are entering the wrong code - No Ignition - Permission denied");
        break;
    case ignit::ARMED:
        ui->ignition_state_icon->setStyleSheet("QLabel {image: url(:/assets/warning.png);}");
        ui->info_ignition->setText("GSE confirmation : Code is correct - ARMED - Ready for Ignition !");

        break;
    case ignit::IGNITION_ON:
        ui->ignition_state_icon->setStyleSheet("QLabel {image: url(:/assets/fire.png);}");
        ui->info_ignition->setText("Ignition circuit active ! Igniters should be burning!");
#ifdef SOUND_ON
        // playSound(takeoff);
#endif
        break;
...
void SecondWindow::initialize_slots_signals()
{
    ...
    connect(ui->xbee_button, SIGNAL(pressed()), this, SLOT(xbee_clicked()));
    connect(ui->reset_button, SIGNAL(pressed()), this, SLOT(reset_button_pressed()));
    connect(ui->ready_ignit_button, SIGNAL(pressed()), this, SLOT(ignite_clicked()));
    connect(ui->change_theme, SIGNAL(pressed()), this, SLOT(theme_change_clicked()));
    connect(ui->file_transmission_button, SIGNAL(pressed()), this, SLOT(file_transmission_pressed()));
    connect(ui->valve_button,SIGNAL(pressed()), this, SLOT(valve_control()));
    connect(ui->play_music,SIGNAL(pressed()), this, SLOT(play_music_pressed()));
    connect(ui->rssi_button,SIGNAL(pressed()), this, SLOT(rssi_get_pressed()));
    connect(ui->send_msg,SIGNAL(pressed()), this, SLOT(send_msg_pressed()));
    connect(ui->clear_image,SIGNAL(pressed()), this, SLOT(clear_image_pressed()));
    connect(ui->PL_image_abort,SIGNAL(pressed()), this, SLOT(image_abort_pressed()));
    connect(ui->ftx_missing_request,SIGNAL(pressed()), this, SLOT(ftx_missing_request_pressed()));
    connect(ui->ftx_save_file,SIGNAL(pressed()), this, SLOT(ftx_save_file_pressed()));
}

void SecondWindow::refresh_telemetry()
{
    ui->altitude_panel_telemetry->setText(qstr(connector->getData<float>(T_ALTITUDE)));
    ui->accel_x_panel->setText(qstr(connector->getData<float>(T_ACCELEROMETER_X)));
    ui->accel_y_panel->setText(qstr(connector->getData<float>(T_ACCELEROMETER_Y)));
    ui->accel_z_panel->setText(qstr(connector->getData<float>(T_ACCELEROMETER_Z)));
    ui->euler_x_panel->setText(qstr(connector->getData<float>(T_EULER_X)));
    ui->euler_y_panel->setText(qstr(connector->getData<float>(T_EULER_Y)));
    ui->euler_z_panel->setText(qstr(connector->getData<float>(T_EULER_Z)));
    ui->pressure_panel->setText(qstr(connector->getData<float>(T_PRESSURE)));
    ui->speed_panel->setText(qstr(connector->getData<float>(T_SPEED)));
    ui->temperature_panel->setText(qstr(connector->getData<float>(T_TEMPERATURE)));
}

void SecondWindow::refresh_gps()
{
    ui->altitude_lcd_gps->display(qstr((int) connector->getData<float>(GPS_ALTITUDE)));
    ui->altitude_max_lcd_m->display(qstr((int) connector->getData<float>(ALTITUDE_MAX)));
    ui->longitude_panel->setText(qstr(connector->getData<float>(GPS_LONGITUDE)));
    ui->latitude_panel->setText(qstr(connector->getData<float>(GPS_LATITUDE)));
    ui->hdop_panel->setText(qstr(connector->getData<float>(GPS_HDOP)));
    ui->sat_nbr_panel->display(qstr(connector->getData<uint8_t>(GPS_SAT_NBR)));
}
...
void SecondWindow::refresh_com()
{
    ...
    ui->received_pack_cnt_panel->setText(qstr(connector->getData<uint64_t>(TOTAL_RX_PACKET_CTR)));
    ...
    ui->miss_panel->setText(qstr(calculate_misses_in_last_2()));
    //this->last_refresh_panel->setText(tbuffer);
    uint32_t coef(1000.0 / REFRESH_RATE);
    ui->all_packet_rate->setValue((connector->eatData<uint32_t>(PACKET_CTR_ALL, 0) * coef));
    ui->AV_packet_rate->setValue((connector->eatData<uint32_t>(PACKET_CTR_AV, 0) * coef));
    ui->PL_packet_rate->setValue((connector->eatData<uint32_t>(PACKET_CTR_PL, 0) * coef));
    ui->GSE_packet_rate->setValue((connector->eatData<uint32_t>(PACKET_CTR_GSE, 0) * coef));
    ui->PP_packet_rate->setValue((connector->eatData<uint32_t>(PACKET_CTR_PP, 0) * coef));

    ui->corrupted_panel->setText(qstr(connector->getData<uint64_t>(CORRUPTED_PACKET_CTR)));
    ...
    ui->rssi_value->display(-1* (int) connector->getData<uint8_t>(ui_interface::RSSI_VALUE));
...
void SecondWindow::refresh_file_transmission()
{
    ...
    ui->transmitter_state_panel->setText(QString::fromStdString(getStateName(state)));
    state = connector->getData<FileTransmissionStates>(ui_interface::FILE_TRANSMISSION_MY_STATE);
    ui->receiver_state_panel->setText(QString::fromStdString(getStateName(state)));
    ...
    ui->file_transmission_progress_bar->setMaximum(packetTotNbr);
    ui->file_transmission_progress_bar->setValue(packetNbr);

    if (connector->eatData<bool>(ui_interface::FTX_FILE_TX_SENT, false)) {
        FTX_beginning_time = std::time(nullptr);
        ui->PL_state_file_Tx_sent->setStyleSheet("QLabel {image: url(:/assets/correct.png);}");
    }
    if (connector->eatData<bool>(ui_interface::FTX_PL_RESPONSE, false))
        ui->PL_state_payload_response->setStyleSheet("QLabel {image: url(:/assets/correct.png);}");
    if (connector->eatData<bool>(ui_interface::FTX_MISSING_REQUEST_SENT, false))
        ui->PL_state_missing_request_sent->setStyleSheet("QLabel {image: url(:/assets/correct.png);}");
    if (connector->eatData<bool>(ui_interface::FTX_ALL_RECEIVED, false))
        ui->PL_state_all_received->setStyleSheet("QLabel {image: url(:/assets/correct.png);}");
    if (connector->eatData<bool>(ui_interface::FTX_ACK_SENT, false)) {
        FTX_beginning_time = 0;
        ui->PL_state_ack_sent->setStyleSheet("QLabel {image: url(:/assets/correct.png);}");
    }

    if (connector->eatData<bool>(FILE_TRANSMISSION_ALL_RECEIVED, false)) {
        ui->PL_image_display->setAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        QPixmap img(QString::fromStdString(connector->getImgPLfilename()));
        ui->PL_image_display->setPixmap(img);
    }
    ...
    if (FTX_beginning_time != 0) {
        ...
        char buf[32];
        std::strftime(buf, 32, "%T", tt);
        ui->ftx_beginning_time->setText(buf);

        // Time left
        if (ui->PL_packet_rate->value() != 0) {
            t = (packetTotNbr - packetNbr) / ui->PL_packet_rate->value();
            tt = gmtime(&t);
            std::strftime(buf, 32, "%T", tt);
            ui->ftx_time_left->setText(buf);
        }
    }
    // Packet counter
    auto Rx_packet_ctr(connector->getData<uint64_t>(ui_interface::FTX_RX_PACKET_CTR));
    ui->ftx_packet_ctr->setText(qstr((Rx_packet_ctr > 0) ? Rx_packet_ctr - 1 : 0));
    ui->ftx_missed_packets->setText(qstr((packetNbr > Rx_packet_ctr) ? packetNbr - Rx_packet_ctr - 1 : 0));
}
...
void SecondWindow::play_music_pressed() {
    ...
    if (!musicON_) {
        ...
        ui->verticalSlider_11->setValue(0);
        ui->verticalSlider_12->setValue(0);
        ui->verticalSlider_13->setValue(0);
        ui->verticalSlider_14->setValue(0);
        ui->verticalSlider_15->setValue(0);
#ifdef SOUND_ON
        m_player->stop();
#endif
    }
...
void SecondWindow::send_msg_pressed() {
    ...
    std::cout << ui->password_line_edit->text().toStdString() << std::endl;
}

void SecondWindow::clear_image_pressed() {
    ui->PL_image_display->clear();
    ui->PL_state_file_Tx_sent->setStyleSheet("QLabel {image: url(:/assets/refresh.png);}");
    ui->PL_state_payload_response->setStyleSheet("QLabel {image: url(:/assets/refresh.png);}");
    ui->PL_state_missing_request_sent->setStyleSheet("QLabel {image: url(:/assets/refresh.png);}");
    ui->PL_state_all_received->setStyleSheet("QLabel {image: url(:/assets/refresh.png);}");
    ui->PL_state_ack_sent->setStyleSheet("QLabel {image: url(:/assets/refresh.png);}");
    ...
    ui->ftx_beginning_time->setText("...");
    ui->ftx_time_left->setText("...");
